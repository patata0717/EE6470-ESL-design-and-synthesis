...
#define num_of_PEs 4
uint32_t count = 0;
uint32_t backup_cnt[num_of_PEs] = {0, 0, 0, 0}; // backup count for each PE
/* ----------------‐ global state for DMA ------------------------ */
static uint8_t    dma_busy = 0;             
static uint8_t    dma_read = 0;             

/* ----------------‐ PE state------------------------------ */
static uint8_t pe_idle[num_of_PEs] = {1, 1, 1, 1};
...
static void irq_PE2(void) {irq_PE(2);}
static void irq_PE3(void) {irq_PE(3);}
...
void init(void)
{
    ...
    register_interrupt_handler(7, irq_PE2);
    register_interrupt_handler(8, irq_PE3);
}
...
int main(void)
{
    ...
    value =  (uint32_t*)malloc((num_items + 1) * sizeof(uint32_t));
    weight = (uint32_t*)malloc((num_items + 1) * sizeof(uint32_t));
    ...
    while (1) {
        ...
        int all_idle = pe_idle[0] && pe_idle[1] && pe_idle[2] && pe_idle[3];
        int all_busy = !pe_idle[0] && !pe_idle[1] && !pe_idle[2] && !pe_idle[3];
        if (all_busy) {
            printf("wfi\n");
            asm volatile("wfi");
