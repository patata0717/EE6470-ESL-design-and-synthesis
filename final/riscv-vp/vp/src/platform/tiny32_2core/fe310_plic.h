...
struct FE310_PLIC : public sc_core::sc_module, public interrupt_gateway {
	...
	void gateway_trigger_interrupt(uint32_t irq_id) {
    	std::cout << "[vp::plic] gateway_trigger_interrupt called with irq_id = " << irq_id << std::endl;
		...
		if (trace_mode) std::cout << "[vp::plic] pending_interrupts[" << idx << "] = 0x" << std::hex << pending_interrupts[idx] << std::dec << std::endl;
