...
void Testbench::do_LoG() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      if (x == 0) {
        ...
        for (u = -2; u <= 2; u++) {
          for (v = -1; v <= 1; v++) {
            ...
            data.uc[0] = INTENSITY;
            mask[0] = 0xff;
            initiator.write_to_socket(LOG_FILTER_R_ADDR, mask, data.uc, 4);
      ...
      int output_num=0;
      while(!done){
        initiator.read_from_socket(LOG_FILTER_CHECK_ADDR, mask, data.uc, 4);
        output_num = data.sint;
        if(output_num>0) done=true;
      }
      wait(10 * CLOCK_PERIOD, SC_NS);
      initiator.read_from_socket(LOG_FILTER_RESULT_ADDR, mask, data.uc, 4);
      total = data.sint;
